...
export async function initPaypalPaymentSession(args: InitCheckoutSessionArgs): Promise<PaymentSession> {
...
export async function completePaypalPaymentSession(uuid: string){
  ...
  if(r.errors?.length || !r.data.order || r.data.order.paymentProvider != PaymentProvider.paypal)
    return false;
  ...
  if(response.result?.status == "COMPLETED"){
    ...
    if(r2.errors?.length || !r2.data.setOrderStatusPaid || r2.data.setOrderStatusPaid.errors?.length)
      return false;
    else
      return true;

...
export function getShippingCost(
  ...
  if (
    ...
    shippingMethod.freeShippingMin < getSubTotal(lineItems)
  )
    return 0;
  else return parseFloat(shippingMethod.price as any);
}
...
export async function getTotalVat(lineItems: LineItemInput[]): Promise<number> {
  const variants = await context()
    ...
    .find({ _id: { $in: lineItems.map((a) => a.variant) } })
    .toArray();
...
export function paymentProviderFromPaymentMethod(

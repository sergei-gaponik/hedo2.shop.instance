...
async function updateAllCategories(): Promise<InstanceResponse> {
  ...
  const dereference = (id) => {
    return _categories.filter((a) => a.parent?._id == id).map((a) => f(a));
  };

  const f = (category) => {
    const filters =
      category.filters ||
      category.andCondition
  ...
  const dereferenceParent = (category) => {
    if (category.parent) {
      ...
      if (c) {
        ...
        if (category.parent.children) delete category.parent.children;
      }
    }
  ...
  if (insertedCount != categories.length) throw new Error();

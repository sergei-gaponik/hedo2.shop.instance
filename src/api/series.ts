...
export async function getSeriesForBrand(args): Promise<InstanceResponse> {
  ...
  if (!handle) {
    if (seriesHandle) {
      const _series = await context()
        .mongoDB.collection("series")
        .findOne({ handle: seriesHandle });

      if (!_series) return { data: { series: [] } };

      handle = _series.brand.handle;
    } else if (productHandle) {
      const _product = await context()
        .mongoDB.collection("products")
        .findOne({ handle: productHandle });

      if (!_product) return { data: { series: [] } };
